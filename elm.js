...
var $author$project$Main$CountingUp = {$: 'CountingUp'};
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{
			direction: $author$project$Main$CountingUp,
			lastPosix: $elm$time$Time$millisToPosix(0),
			running: false,
			time: 0
		},
...
var $author$project$Main$CountingDown = {$: 'CountingDown'};
			...
			switch (msg.$) {
						...
						_Utils_update(
							model,
							{
								direction: function () {
									...
									if (_v1.$ === 'CountingDown') {
										return $author$project$Main$CountingUp;
									} else {
										return $author$project$Main$CountingDown;
									}
								}()
							}),
						$elm$core$Platform$Cmd$none);
				default:
					...
					if (!model.running) {
						...
						var isFirstTick = !$elm$time$Time$posixToMillis(model.lastPosix);
...
var $author$project$Main$view = function (model) {
	var totalSeconds = $elm$core$Basics$round(model.time);
	...
	var timeDisplay = (hours > 0) ? (padZero(hours) + (':' + (padZero(minutes) + (':' + padZero(seconds))))) : (padZero(minutes) + (':' + padZero(seconds)));
	return {
